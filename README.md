...
| 安全与上下文 | PII 双层脱敏（手机号 / 18 位身份证 / 邮箱）：输入入口先掩码再落盘/展示（身份证保留地区码与尾4位、邮箱只留顶级域，旧会话加载时补脱敏），3 个 PIIMiddleware 在模型调用前按身份证→手机→邮箱顺序兜底；SummarizationMiddleware 超 24 轮自动摘要压缩 |
